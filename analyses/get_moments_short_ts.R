...
# The method breaks after 250 species in the case of BCI, so I skip those
# estimates to estimate the breakpoint with the mu. For T/r, the error does not
# fluctuate as much and can be left untouched. 
x <- x[1:250] 
...
x <- unname(unlist(as.data.frame(data)[,order(colSums(data), decreasing = T)[1:break_at]]))
